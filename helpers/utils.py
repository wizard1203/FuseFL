...
def add_shape(image, shape, location, size, color):
    ...
    if shape == 'circle':
        ...
        # 绘制星形较复杂，需要计算五个顶点的位置
        points = [(x + int(p[0] * size / 30), y + int(p[1] * size / 30)) for p in 
                [(15, 0), (19, 10), (30, 10), (21, 17), 
                (24, 30), (15, 22), (6, 30), (9, 17), 
                (0, 10), (11, 10)]]
        draw.polygon(points, fill=color)
    elif shape == 'diamond':
        draw.polygon([x, y + size / 2, x + size / 2, y, x + size, y + size / 2, x + size / 2, y + size], fill=color)
    elif shape == 'heart':
        # 绘制心形较复杂，这里是简化的示例
        ...
        thickness = size // 4
        draw.rectangle([x + thickness, y, x + size - thickness, y + size], fill=color)
        draw.rectangle([x, y + thickness, x + size, y + size - thickness], fill=color)
    elif shape == 'pentagon':
        angle = 2 * np.pi / 5
        points = [(x + size // 2 + size // 2 * np.sin(i * angle), y + size // 2 + size // 2 * np.cos(i * angle)) for i in range(5)]
        draw.polygon(points, fill=color)
    elif shape == 'hexagon':
        angle = 2 * np.pi / 6
        points = [(x + size // 2 + size // 2 * np.sin(i * angle), y + size // 2 + size // 2 * np.cos(i * angle)) for i in range(6)]
        draw.polygon(points, fill=color)
    return image
...
def progressive_test(model, test_loader, test_client_idx, device):
    model.eval()
    test_loss = 0
    correct = 0
    model.to(device)
    with torch.no_grad():
        for data, target in test_loader:
            # data, target = data.cuda(), target.cuda()
            data, target = data.to(device), target.to(device)
            output = model(data, str(test_client_idx))
            test_loss += F.cross_entropy(output, target, size_average=False).item()  # sum up batch loss
            pred = torch.max(output, 1)[1]
            correct += pred.eq(target.view_as(pred)).sum().item()
